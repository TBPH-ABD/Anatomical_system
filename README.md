...
The explanation is generated by Claude (`claude-opus-5`) through `POST /api/explain`, implemented once in [`server/explain.ts`](server/explain.ts) and served two ways: as a Vercel function ([`api/explain.ts`](api/explain.ts)) in production, and through a dev middleware in [`vite.config.ts`](vite.config.ts) locally. The browser only ever sends the names of the selected structure; the API key stays on the server.
